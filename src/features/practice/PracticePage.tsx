    ...
    <div className={styles.page}>
      ...
      {step === "type" && <>
        <section className={styles.segmented} aria-label="연습 방식">
          <button onClick={() => { setPracticeType("free_chat"); setScenarioId(""); setStep("persona"); }}>자유채팅</button>
          <button onClick={() => { setPracticeType("scenario"); setStep("scenario"); }}>시나리오</button>
          <button onClick={() => navigate("/interview")}>이력서 기반 면접</button>
        </section>
      </>}
        ...
        {personas.data?.items.length ? (
          <div className={styles.cardGrid}>
            {personas.data.items.map((persona) => (
              <button key={persona.id} className={styles.selectCard} disabled={createRoom.isPending} onClick={() => createRoom.mutate(persona.id)}>
        ...
        <section aria-labelledby="scenario-heading">
          ...
          <div className={styles.scenarioList}>
            {scenarios.data?.items.map((scenario) => (
              <button key={scenario.id} className={styles.scenarioCard} aria-pressed={scenario.id === scenarioId} onClick={() => setScenarioId(scenario.id)}>
          ...
          {(selectedScenario.error || createRoom.error) && <div className={styles.error} role="alert">{(selectedScenario.error ?? createRoom.error)?.message}</div>}
          <div className={styles.stickyAction}><button className={styles.primaryButton} disabled={!canStart || createRoom.isPending} onClick={() => createRoom.mutate(undefined)}>{createRoom.isPending ? "대화방 준비 중…" : "대화 시작"}</button></div>
        </section>
      )}
    </div>
  );
